...
首次发布和后续升级使用同一个脚本。升级时脚本会识别 `/opt/rainbow-cats/current` 对应的受管容器，继续完成全套副本验证，再替换 `3101` 上的旧容器；如果候选版本启动或公网检查失败，会自动恢复上一镜像。非 Rainbow-Cats 进程占用 `3101` 时仍会拒绝发布。
...
首次发布生成的回退脚本会启动旧 systemd 服务、恢复原 Caddyfile 并关闭新容器。后续升级生成的回退脚本会重新启动上一容器镜像，并把 `/opt/rainbow-cats/current` 指回上一发布目录。数据库升级只新增表、字段和索引，因此正常回退不恢复数据库转储；数据库备份仍保存在同一目录，用于灾难恢复。
