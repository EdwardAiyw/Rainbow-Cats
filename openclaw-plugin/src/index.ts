...
export function trustedIdentity(context: ToolContext): TrustedIdentity | null {
...
export function directSenderFromSessionKey(context: ToolContext) {
...
function allowedAgentAccounts() {
  return new Set(
    String(process.env.RAINBOW_CATS_AGENT_ACCOUNT_IDS || "")
      .split(",")
...
async function callRainbow<T>(
  ...
  if (!token) throw new Error("Rainbow-Cats 内部令牌未配置");
  ...
  if (!response.ok || !envelope.ok || envelope.data === null) {
    ...
    if (envelope.error?.code) error.name = envelope.error.code;
    throw error;
  }
  ...
  tools: (tool) => [
    tool({
      ...
      factory: ({ api, toolContext }) => {
        ...
        api.logger.info(
          `[rainbow-cats] tool-context channelMatch=${toolContext.messageChannel === CHANNEL} ` +
        ...
        return {
          ...
          description: "使用用户刚从 Rainbow-Cats 网页生成的 6 位绑定码，绑定当前微信身份。",
          parameters: LinkSchema,
          async execute(_id, params: { code: string }, signal) {
      ...
      factory: ({ toolContext }) => {
        ...
        return {
          ...
          async execute(_id, _params, signal) {
            const data = await callRainbow<Record<string, unknown>>("/api/internal/openclaw/context", identity, {}, signal);
            return jsonContent({
              linked: true,
              ...data,
              promptGuidelines: [
    ...
    tool({
      ...
      factory: ({ toolContext }) => {
        ...
        return {
          ...
          description: "创建待确认的任务、礼物、账目或日程动作。必须把返回的 6 位确认码告诉当前用户，不能代替用户确认。",
          parameters: ProposeSchema,
          async execute(_id, params: { action: string; payload: Record<string, unknown> }, signal) {
            const data = await callRainbow<Record<string, unknown>>(
              "/api/internal/openclaw/proposals",
              identity,
    ...
    tool({
      ...
      factory: ({ toolContext }) => {
        ...
        return {
          ...
          description: "仅在当前用户明确回复正确的 6 位确认码后执行对应提案。不要猜测或复用确认码。",
          parameters: ConfirmSchema,
          async execute(_id, params: { proposalId: string; confirmationCode: string }, signal) {
            const data = await callRainbow<Record<string, unknown>>(
              "/api/internal/openclaw/confirm",
              identity,
              {
